<template>
  <VueDatePicker v-model="modelValue" format="dd MMM yyyy" :inputClassName="className" menuClassName="shopperz-menu"
                 :placeholder="inputStyle == 'filter' ? '' : $t('label.select_date_range')" :presetDates="presetDates"
                 :enableTimePicker="false" :autoApply="true" :range="range" utc="false"
  ...
  computed: {
    className(state) {
      if (state.inputStyle == 'box') {
        ...
        return 'shopperz-input box'
      } else if (state.inputStyle == 'read') {
        const date = new Date();
        const startDate = new Date(date.getFullYear(), date.getMonth(), 1);
        const endDate = new Date(date.getFullYear(), date.getMonth() + 1, 0);
        this.modelValue = [startDate, endDate];
        return 'shopperz-input read'
      } else if (state.inputStyle == 'filter') {
        return 'shopperz-input filter'
      } else return 'shopperz-input'
    }
  },
}
</script>
